...
function sanitizeEntry(body) {
  const {
    tmdbId, title, posterPath, backdropPath, year, rating, overview, type,
  } = body;
  ...
  return {
    data: {
      ...
      rating: rating !== undefined ? Math.min(10, Math.max(0, Number(rating) || 0)) : undefined,
...
export const addWatchlistEntry = async (req, res) => {
  if (!isDbConnected()) {
